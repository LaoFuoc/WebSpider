...
class QianChengWuYouSpider(object):
        ...
        while True:
            if not self.q.empty():
                ...
                for result in results:
                    ...
                    news['工作经验'],news['学历要求'],news['招聘人数'],news['公司福利'],news['职位信息'] = self.parse_two_page(two_url)
                    print(news)
    ...
    def parse_two_page(self, two_url):
        ...
        FenGe = '\n'#以换行符分隔
        results = FenGe.join(results)
        try:
            welfare = html.xpath('/html/body/div[3]/div[2]/div[2]/div/div[1]/div/div//text()')  # 公司福利
            welfare = [x.strip() for x in welfare if x.strip() != '']
            FenGe = ','  # 以,分隔
            welfare = FenGe.join(welfare)
        except:
            welfare = ''

        return workyear,edu,ZhaoRen,welfare,results

...
Top21_codes = ['736', '702', '854', '635', '765', '801', '763', '530', '600', '538', '749', '653', '531', '681', '664', '682', '639', '703', '599', '565', '551', '613']

class ZhiLianSpider(object):
    def __init__(self):
        self.one_url = 'https://fe-api.zhaopin.com/c/i/sou?start={}&pageSize=90&cityId={}&salary=0,0&workExperience=-1&education=-1&companyType=-1&employmentType=-1&jobWelfareTag=-1&kw=java&kt=3'
        # self.two_url = ''
        self.q = Queue()
        ...
        while True:
            if not self.q.empty():
                ...
                for job in html['data']['results']:
                    jobName = job['jobName']  # 工作岗位
                    company = job['company']['name']  # 公司名称
                    type = job['company']['type']['name']  # 融资类型
                    try:
                        size = job['company']['size']['name']  # 公司规模
                    except KeyError as  e:
                        size = ''
                    city = job['city']['items'][0]['name']  # 城市
                    try:
                        city_name = job['city']['items'][1]['name']  # 地区
                    except:
                        city_name = city
                    updateDate = job['updateDate']  # 发布时间
                    salary = job['salary']  # 薪水
                    eduLevel = job['eduLevel']['name']  # 学历要求
                    try:
                        workingExp = job['workingExp']['name']  # 工作经验
                    except:
                        workingExp = '无经验'
                    positionURL = job['positionURL']  # 职位链接
                    welfare = job['welfare']  # 公司福利
                    item = {"工作岗位": jobName, "城市": city, '地区': city_name, "薪水": salary, "学历要求": eduLevel,
                            "工作经验": workingExp,
                            "公司名称": company, "融资类型": type, "公司规模": size, "发布时间": updateDate, "公司福利": welfare,
                            "职位链接": positionURL}

                    # 拼接二级地址,获取职责和要求
                    # two_url = self.two_url.format(positionURL)
                    # more_message = self.parse_two_page(two_url)
    ...
    # 获取总页数
    def get_numbers(self,city_code):
        url = self.one_url.format(0,city_code)
        html = self.get_page(url)
        numbers = int(html['data']['count']) // 90 + 1

        return numbers

    def main(self,citycode):
        # one_url入队列
        number = self.get_numbers(citycode)
        for page in range(1, number + 1):
            one_url = self.one_url.format((page-1)*90,citycode)
            self.q.put(one_url)
...
if __name__ == '__main__':
    ...
    db = client.DuoXianCheng  # 创建数据库
    collection = db.JiXian3
    spider = ZhiLianSpider()
    for citycode in Top21_codes[:5]:
        try:
            spider.main(citycode)

...
print(len(citys),len(citys_codes))
def get_json(url):
    headers = {
...
if __name__ == '__main__':
    ...
    collection = db.City[job]
    num = 0
    for citycode in citys_codes:
        url = 'https://fe-api.zhaopin.com/c/i/sou?pageSize=90&cityId={}&workExperience=-1&education=-1&companyType=-1&employmentType=-1&jobWelfareTag=-1&kw={}&kt=3'.format(citycode, job)
        resp = get_json(url)
        resp = resp.json()
        condition = resp['data']
        if ('count' in condition):
            counts = resp['data']['count']
            print(citys[citycode] +' 代码 '+citycode+ "  总共%s条信息" % counts)
            num += counts
